...
const AUTH_ERROR = { statusCode: 401, body: { error: 'Mot de passe admin invalide (ou ADMIN_PASSWORD non configuré).' } };

async function adminStats(key) {
  if (!checkAdmin(key)) return AUTH_ERROR;
  const db = supabase();
...
async function adminUpload(key, slot, dataBase64, contentType) {
  if (!checkAdmin(key)) return AUTH_ERROR;
  if (!ASSET_SLOTS.includes(slot)) {
...
async function adminDeleteAsset(key, slot) {
  if (!checkAdmin(key)) return AUTH_ERROR;
  const db = supabase();
  if (!db) return { statusCode: 500, body: { error: 'Supabase non configuré.' } };
  const { data: files } = await db.storage.from('site-assets').list('', { limit: 100 });
...
async function adminSetSetting(key, name, value) {
  if (!checkAdmin(key)) return AUTH_ERROR;
